...
class RecordAudioButton extends StatefulWidget {
  const RecordAudioButton({super.key, required this.onRecordingFinishedCallback, required this.heroTag, this.width = 60, this.height = 60});
    ...
    return GestureDetector(
      ...
      onLongPressEnd: (LongPressEndDetails details) async{
        if (details.localPosition.dx < -30) {
          await audioService.cancelWaveRecord().then((value) {debugPrint("Audio recording has been canceled");});
          setState(() {});
          return;
        }
